...
$(document).ready(function () {
    ...
    var id = setInterval(function () {
        ...
        $(document).on('click', '#customer_list .icon-credit-card', function () {
            cust_id1 = parseInt($(this).attr("cust_id"));
            customerFeeInfo(cust_id1)
            // console.log(cust_id)
        })
        ...
        buttons[i18next.t("system.save")] = function () {
            // console.log(1)
            customerFeeSave(cust_id1);

        };
        buttons[i18next.t("system.cancel")] = function () {
            $(this).dialog("close");
        };
        $('#divCustomerFee').dialog({
            autoOpen: false,
            width: 480,
            buttons: buttons
        });

        $('#hasCard').change(function () {
            // alert(this.value)
            if (this.value == 1) {
                $('#cardPackage').show()
            } else {
        ...
        $('#plus').on('click', function () {
            ...
            var cardPackage = $('#cardPackage').val();
            var giftTime = parseInt($('#giftTime').val() || 0);
            ...
            if (hasCard == 1 && cardPackage == 0) {
                _alert('请选择卡类型');
                return;
            }
            if (isNaN(price) || isNaN(giftTime) || isNaN(limit)) {
                _alert('请输入正确的数字');
                return;
            }
            ...
            // var isExist = false;
            // for (var i = 0; i < servicePlan.length; i++) {
            //     if (servicePlan[i]['cardPackage'] == cardPackage) {
            //         isExist = true;
            //         break;
            //     }
            // }
            ...
            // }
            $.ajax({
                url: "http://api.chease.cn/next_id", //后台处理程序
                type: "get", //数据发送方式
                ...
                success: function (res) {
                    ...
                    limit == 0 ? null : servicePlanObj.limit = limit;
                    servicePlan.push(servicePlanObj);
                    serviceRender();
                }
            })
            // $.ajax({
...
function customerFeeInfo(objectId) {
    var query_json = {
        uid: objectId
    };
    wistorm_api._get('customer', query_json, 'name,contact,tel,treePath,other,treepath', auth_code, true, function (json) {
        debugger;
        if (json.status_code === 0 && json.data) {
            AllCustomerFee(json.data.treePath, objectId);
            $("#divCustomerFee").dialog("option", "title", i18next.t('customer.fee_adjust'));
            $("#divCustomerFee").dialog("open");
            $('#_current_month_fee').val(0);
            $('#_current_year_fee').val(0);
            $('#_2m_year_fee').val(0);
            $('#_5m_year_fee').val(0);
            $('#_30m_year_fee').val(0);
            $('#_100m_year_fee').val(0);
            servicePlan = [];
            if (json.data.other) {
                servicePlan = json.data.other.servicePlan || [];
                serviceRender();
                $('#_current_month_fee').val(json.data.other.parentMonthFee || 0)
                $('#_current_year_fee').val(json.data.other.parentYearFee || 0)
                if (json.data.other.flow) {
                    if (json.data.other.flow[2]) {
                        $('#_2m_year_fee').val(json.data.other.flow[2].parentYearFee || 0);
                    }
                    if (json.data.other.flow[5]) {
                        $('#_5m_year_fee').val(json.data.other.flow[5].parentYearFee || 0);
                    }
                    if (json.data.other.flow[30]) {
                        $('#_30m_year_fee').val(json.data.other.flow[30].parentYearFee || 0);
                    }
                    if (json.data.other.flow[100]) {
                        $('#_100m_year_fee').val(json.data.other.flow[100].parentYearFee || 0);
                    }
                }
            }
        }
...
//基准费率
function AllCustomerFee(tree_path, objId) {
    var uids = ''
    tree_path.split(',').forEach(e => {
        if (e) {
            uids += e + '|';
        }
    });
    ...
    wistorm_api._list('customer', query_json, 'objectId,name,treePath,parentId,uid,custType,other', 'custType,name', '-createdAt', 0, 0, 1, -1, auth_code, true, function (json) {
        var parentMonthFee = parentYearFee = parentYearFee2 = parentYearFee5 = parentYearFee30 = parentYearFee100 = 0;
        if (json.status_code == 0) {
            json.data.forEach(ele => {
                if (ele.uid == objId) {
                    // parentMonthFee += parseFloat(ele.other.parentMonthFee || 0);
                    // parentYearFee += parseFloat(ele.other.parentYearFee || 0)
                } else {
                    // parentMonthFee += parseFloat(ele.other.parentMonthFee || 0) + parseFloat(ele.other.monthFee || 0);
                    // parentYearFee += parseFloat(ele.other.parentYearFee || 0) + parseFloat(ele.other.yearFee || 0)
                    // parentMonthFee += (ele.other ? parseFloat(ele.other.parentMonthFee || 0) : 0) + (ele.other ? parseFloat(ele.other.monthFee || 0) : 0)
                    // parentYearFee += (ele.other ? parseFloat(ele.other.parentYearFee || 0) : 0) + (ele.other ? parseFloat(ele.other.yearFee || 0) : 0)
                    if (ele.other) {
                        parentMonthFee += parseFloat(ele.other.parentMonthFee || 0) + parseFloat(ele.other.monthFee || 0);
                        parentYearFee += parseFloat(ele.other.parentYearFee || 0) + parseFloat(ele.other.yearFee || 0);
                        if (ele.other.flow) {
                            if (ele.other.flow[2]) {
                                parentYearFee2 += parseFloat(ele.other.flow[2].parentYearFee || 0) + parseFloat(ele.other.flow[2].yearFee || 0);
                            }
                            if (ele.other.flow[5]) {
                                parentYearFee5 += parseFloat(ele.other.flow[5].parentYearFee || 0) + parseFloat(ele.other.flow[5].yearFee || 0);
                            }
                            if (ele.other.flow[30]) {
                                parentYearFee30 += parseFloat(ele.other.flow[30].parentYearFee || 0) + parseFloat(ele.other.flow[30].yearFee || 0);
                            }
                            if (ele.other.flow[100]) {
                                parentYearFee100 += parseFloat(ele.other.flow[100].parentYearFee || 0) + parseFloat(ele.other.flow[100].yearFee || 0);
                            }
                        }
                    }
                }
        ...
        $('#_parent_month_fee').text(parentMonthFee);
        $('#_parent_year_fee').text(parentYearFee);
        $('#_2mparent_year_fee').text(parentYearFee2);
        $('#_5mparent_year_fee').text(parentYearFee5);
        $('#_30mparent_year_fee').text(parentYearFee30);
        $('#_100mparent_year_fee').text(parentYearFee100);
        // console.log(json)
    })
}


function customerFeeSave(objectId) {
    var month_fee = parseFloat($("#_current_month_fee").val() || 0);
    var year_fee = parseFloat($("#_current_year_fee").val() || 0);

    var year_fee2M = parseFloat($('#_2m_year_fee').val() || 0);
    var year_fee5M = parseFloat($('#_5m_year_fee').val() || 0);
    var year_fee30M = parseFloat($('#_30m_year_fee').val() || 0);
    var year_fee100M = parseFloat($('#_100m_year_fee').val() || 0);

    var query_json = {
        uid: objectId
    };
    var update_json = {
        'other.parentMonthFee': month_fee,
        'other.parentYearFee': year_fee,
        'other.flow.2.parentYearFee': year_fee2M,
        'other.flow.5.parentYearFee': year_fee5M,
        'other.flow.30.parentYearFee': year_fee30M,
        'other.flow.100.parentYearFee': year_fee100M,
        'other.servicePlan': servicePlan
    };
    debugger;
    wistorm_api._update('customer', query_json, update_json, auth_code, true, function (json) {
        if (json.status_code == 0) {
            $("#divCustomerFee").dialog("close");
        } else {
...
var serviceRender = function () {
    $('#combo').empty();
    servicePlan.forEach((ele, i) => {
        var deleteId = 'comboRemove' + i
        var htmlTemplate =
            `<div class="control-group">
                <span>${ele.name} </span>
                <span>${ele.price}元</span>
                <span>赠送${ele.giftTime}个月</span>
                ${ele.limit ? `<span>最大购买量${ele.limit}份</span>` : `<span></span>`}
                <i id=${deleteId} name=${i} class=" icon-remove-sign" title="comboRemove" style="font-size: 16px;color: red; position: relative;top: 1px;cursor: pointer;"></i>
            </div>`
        $('#combo').append(htmlTemplate);
        $('#' + deleteId).click(function () {
            servicePlan.splice($(this).attr('name'), 1);
            serviceRender();
        })
    })
}
...
var customerQuerySuccess = function (json) {
    ...
    // var MM = new csMenu(document.getElementById("customerTree"), document.getElementById("Menu1"));
    if (uid > 0) {
        var treeObj = $.fn.zTree.getZTreeObj("customerTree");
...
var querySuccess = function (json) {
    ...
    var _columns = [
        ...
        {
            "mData": null, "sClass": "center", "bSortable": false, "fnRender": function (obj) {
                return "<a href='#' title='" + i18next.t("table.edit") + "'><i class='icon-edit' cust_id='" + obj.aData.uid + "'></i></a>&nbsp&nbsp<a href='#' title='" + i18next.t("table.edit") + "'><i class='icon-credit-card' cust_id='" + obj.aData.uid + "'></i></a>&nbsp&nbsp<a href='#' title='" + i18next.t("table.change_parent") + "'><i class='icon-retweet' cust_id='" + obj.aData.uid + "' cust_name='" + obj.aData.name + "'></i></a>&nbsp&nbsp<a href='#' title='" + i18next.t("table.delete") + "'><i class='icon-remove' cust_id='" +
